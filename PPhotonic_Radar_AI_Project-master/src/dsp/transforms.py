...
def compute_range_doppler_map(
    ...
    """
    Computes Agnostic Range-Doppler Map using 2D FFT.
    ...
    # Truncate separate tail if needed
    ...
    # 2. Windowing
    # Broadcast window across both dimensions if desired, usually just range first.
    win_range = get_window(window_func, samples_per_chirp)
    win_doppler = get_window(window_func, n_chirps)
    ...
    # We transpose to multiply columns, or just broadcast carefully
    ...
    rd_shifted = np.fft.fftshift(rd_complex, axes=0) # Shift Doppler (0 freq to center)
    rd_mag = np.abs(rd_shifted)
    
    # Log scale for visualization/AI (preserving dynamic range)
