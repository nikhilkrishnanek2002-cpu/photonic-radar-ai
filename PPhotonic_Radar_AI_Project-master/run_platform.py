...
log_file = os.path.join(log_dir, "system_platform.log")
...
def start_subsystems():
    ...
    def launch_dashboard():
        ...
        try:
            # We use 'app.py' which is in the same directory.
            ...
            # Only if UI enabled or default? Original: always.
            time.sleep(2)
            if state.config.get('ui_enabled', False):
                logger.info("[DASHBOARD] Opening browser...")
                webbrowser.open(dashboard_url)
            else:
                 logger.info("[DASHBOARD] UI running headless. Use --ui to auto-open browser.")
